...
testplot.time<-ggplot(tracts.df, aes(long, lat, group = group, fill = mean_pct_increase)) + scale_fill_gradient2(low ='green', mid = "light gray", high = "red", 
...
testplot.time
#This works? it doesn't look great, though

#reg stuff
test.df<-join(dt_origin_changes_reg,census, by ='id')
test2<-as.data.frame(test.df)
...
pctwhite<-test2$race.white.n/test2$race.total.n
pctasian<-test2$race.asian.n/test2$race.total.n
pcthisp<-test2$race.hispanic.n/test2$race.total.n #this becomes NA in all regressions
