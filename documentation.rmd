...
reg.test<-lm(target ~ pblack + pwhite + pBus) #pblack is significant at 0.1, other two are barely not. what do do?
summary(reg.test)
```
...
testplot.time<-ggplot(test_transit, aes(long, lat, group = group, fill = cut_pct_increase)) + scale_fill_gradient2(low ='dark green', mid = "white", high = "red", 
