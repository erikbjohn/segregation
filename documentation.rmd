...
names(dt_cast)<-c("origin.tract", "dest.tract", "driving_03-2017", "driving_03-2019", "transit_03-2017", "transit_03-2019","walking_03-2017","walking_03-2019")
...
Looking at the data for the change from 2017 to 2019 in weighted travel times, it appears to be normally distributed with a mean of approximately zero. Therefore, it appears to be close to a net neutral effect across the city. However, we may need to consider whether or not the percent change in time travelled is fairly similar for blacks and whites or if there are some discrepencies in how the quicker and slower routes are spread across the city.
...
```
   ...
   The median household income for black residents of Richmond is about $28,000. Based on the regression using the relative percentages of blacks and whites in a census tract, the median income, and the interraction variables, at the median household income for blacks, the approximate change in weighted travel time based on the change in the percentage of blacks in the census tract is -0.081. Therefore, the change in weighted travel time is greatest for very heavily black areas. This follows because the change to commute time from before the bus change to after the routes change is driven only by the bus route changes, and a community that uses buses more will benefit more from improved efficiency.
...
\end{equation}
   ...
   The two main drawbacks of the $D$ measure are the lack of spatial information (distance) between populations and the fact that there is no direction implied in the relationship. This is a problem since two groups can experience different levels of segregation depending on which side they are on. This can be caused for any number of reasons, such as the different means of transportation used. A wealthier group that uses mostly cars, for example, will find it easier to visit other parts of the town compared to a lower income group that relies on a transit system that may not take them where they need to go. Therefore, we need a segregation measurement that accounts for both the spatial as well as the aspatial aspects of segregation.
...
## Wasserstein Measure
   ...
   To calculate the segregation levels, accounting for both spatial and aspatial information, we have used the Wasserstein method, also known as the Earth mover distance. The advantage of using the Wasserstein measurement is that it allows us to account for both the spatial and the aspaital components that can lead to the different effective segregation rates. Additionally, it allows us to look at the asymmetric directional relationships that different groups can experience. This is one of the advantages of using a Wasserstein measurement compared to the methods employed by CURA. They only looked at the physical distances that people had to walk to get to the nearest bus stop and how many other bus stops that each bus stop was connected to, but failed to look at the time. The Wasserstein measurement allows us to recognize that, while people may have to walk a little further to the nearest bus stop, their overall commute time may still go down due to the decreased travel time for buses. It also lets us observe any potential changes to perceived segregation even if no one moves, since it accounts for not only the spatial but also the aspatial aspects.

   Next, in order to look at the directional perceived segregation, we measure dissimilarity through a Wasserstein measure, which will include both the spatial information and has the ability to infer directional relationships in the form of an asymmetric graph. This is a two stage process and requires careful selection of counterfactuals.  We begin with the most simple formulation\footnote{The Wasserstein measure is found in the R  \texttt{transport} package.}.
...
#Conclusion
   ...
   While CURA's study focused on the distances necessary to walk to get to bus stops, it did not account for the overall decreased travel time that the bus changes led to when looking at the impact of the bus route changes on the population. Even though there may need to be additional stops for some routes, the overall efficiency has improved and caused the average travel time to decrease for commuters across the city. This makes it easier for people from low income areas who rely on the transit to travel about the city and enter higher income areas, which can lead to a decreased perceived segregation. This decrease travel time would be felt more strongly by the lower income communities who rely on buses more than by those communities that primarily drive a car to work.
...
https://www.census.gov/quickfacts/fact/table/richmondcityvirginia/PST045218
https://www.wvtf.org/post/bus-redesign-prompts-question-who-public-transportation#stream/0
https://www.richmond.com/news/local/city-of-richmond/activist-files-federal-civil-rights-complaint-against-grtc-over-bus/article_019ce1ab-494f-564d-9bcf-dfcd8abbd803.html
http://ridegrtc.com/media/main/Service_and_Fare_Equity_Analysis_April_2018_Changes_10_11_17.pdf
