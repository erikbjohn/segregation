...
author: "Erik B. Johnson and Santiago Pinto"
...
dists.richmond.raw.location <- 'CleanData/dists.richmond.raw.rds'
...
stargazer(dists.richmond, summary=TRUE, header=FALSE, nobs=FALSE, digits=0, align=T, style='qje', title='Pairwise Distance Summary')
```
Correlation matrix for \textbf{dists.richmond}
```{r results='asis', echo=FALSE}
cols <- names(dists.richmond)[which(lapply(dists.richmond, class)=='numeric')]
correlation.matrix <- cor(dists.richmond[, cols, with=FALSE])
colnames(correlation.matrix) <- str_replace_all(colnames(correlation.matrix), '.meters', '.m')
colnames(correlation.matrix) <- str_replace_all(colnames(correlation.matrix), '.seconds', '.s')
stargazer(correlation.matrix, header=FALSE, digits=3, align=T, style='qje', title='Distance Correlation Matrix')
```
\clearpage
...
We begin by calculating a simple dissimlarity index between two groups $X$ and $Y$ described in Equation \eqref{eq:D}. Where $x_i$ and $y_i$ are the respective population counts in each location $i$. Note again that this measure is inherently aspatial and only uses the tract level census data. Note that the 'nb' term in \texttt{seg} library scales the interaction of the iteraction is normalized to 1 and not appropriate for our application. Additional information on the library can be found at [Stanford Tutorial page](http://stanford.edu/~ejdemyr/r-tutorials/segregation/).
...
```{r dissimilarity}
dissimlarity <- funMeasures.dissimlarity(census)
```
